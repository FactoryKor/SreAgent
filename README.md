...
## 3-A. 월별 예상 비용 시뮬레이션 (정기 진단 + 리포트 수령 기준)

> **기준 단가(공식 요율 페이지): 1 AAU = $0.10 (USD)**
> - Always-on(고정): 4 AAU/시간 × $0.10 = **$0.40/시간/에이전트**
> - Active flow(변동): 소비 AAU × $0.10
> - 아래 금액은 **에이전트 1개** 기준, USD. 환율/리전/할인 계약에 따라 달라질 수 있음.

### (1) 고정 비용 (Always-on) — 사용량과 무관하게 매달 발생

| 기간 | 계산 | 금액 |
|---|---|---|
| 시간당 | 4 AAU × $0.10 | $0.40 |
| 하루(24h) | $0.40 × 24 | $9.60 |
| **한 달(약 730시간)** | $0.40 × 730 | **약 $292** |

> 에이전트를 켜두기만 해도(진단을 안 해도) 월 **약 $290 이상**이 기본으로 발생. Stop해도 이 비용은 계속되며 **Delete해야 중단**됨.

### (2) 작업 1건당 단가 (Active flow)

| 작업 유형 | Claude Opus 4.6 | GPT 5.3 Codex |
|---|---|---|
| 간단 질의(Quick question, ~3.8 / ~1.3 AAU) | ~$0.38 | ~$0.13 |
| **진단 리포트 1건(Incident investigation급, ~35.3 / ~11.7 AAU)** | **~$3.53** | **~$1.17** |
| 진단+수정(Full remediation, ~86.5 / ~30.1 AAU) | ~$8.65 | ~$3.01 |

> "진단 후 리포트 수령" 1회는 대략 **Incident investigation 수준**으로 가정(복잡도에 따라 변동).

### (3) 사용 강도별 월 예상 총액 (고정 $292 + 변동)

**시나리오 A — 가벼움 (주 1회 정기 리포트)**
- 정기 리포트 4회 + 애드혹 질의 20회 + 인시던트 조사 2회 / 월

| 모델 | 변동(Active) | 고정(Always-on) | **월 합계** |
|---|---|---|---|
| Claude Opus 4.6 | ~$28.8 | $292 | **~$321** |
| GPT 5.3 Codex | ~$9.6 | $292 | **~$302** |

**시나리오 B — 중간 (매일 정기 리포트)**
- 정기 리포트 30회 + 애드혹 질의 100회 + 인시던트 조사 10회 / 월
...
**시나리오 C — 무거움 (적극적 자동화 + 수정 작업 포함)**
- 리포트 60회 + 애드혹 질의 300회 + 인시던트 조사 30회 + 진단+수정 10회 / 월
...
- **가벼운 사용에서는 고정비($292)가 비용의 대부분** — 진단을 자주 안 해도 기본요금이 크므로, 여러 워크로드를 **한 에이전트로 통합**하는 것이 유리(에이전트 수 = 고정비 배수).
- **사용이 많아질수록 Active flow(변동비)가 지배적** — 이때는 모델 선택이 큰 영향(GPT가 Opus보다 약 3배 저렴).
- 위 AAU 수치는 공식 문서의 **평균 예시값**이며, 실제 토큰 소비는 리소스 규모/로그량/조사 깊이에 따라 크게 달라질 수 있음.
- **월 지출 상한(500~1,000,000 AAU)** 설정으로 변동비 폭주를 방지 가능(고정비는 별도로 계속됨).
- 자동 생성되는 Application Insights / Log Analytics 수집 비용은 **별도**로 발생할 수 있음.

> **간단 요약**: 에이전트 1개를 상시 켜두면 **최소 월 ~$290(고정)**, 여기에 사용량에 따라
> 가벼움 **~$300대**, 중간 **~$350~470**, 무거움 **~$470~810** 수준으로 추정.
